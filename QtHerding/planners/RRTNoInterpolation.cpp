...
RRTNoInterpolation::RRTNoInterpolation(const base::SpaceInformationPtr &si) : RRT(si)
{
  eps_samples = 0;
}

void RRTNoInterpolation::freeMemory()
{
    if (nn_)
    {
...
RRTNoInterpolation::~RRTNoInterpolation()
{
  Planner::clear();
  sampler_.reset();
  freeMemory();
...
std::queue<double> RRTNoInterpolation::stateToCoordsQueue(base::State *state)
{
...
double RRTNoInterpolation::getEpsilon(base::State *state)
{
  double eps, maxEdge;
  int cycle;
...
double RRTNoInterpolation::getCycleSize(base::State *state)
{
  double eps, maxEdge;
  int cycle;
  runVerification(stateToCoordsQueue(state), eps, maxEdge, cycle);
...
double RRTNoInterpolation::getMaxEdge(base::State *state)
{
  double eps, maxEdge;
  int cycle;
  runVerification(stateToCoordsQueue(state), eps, maxEdge, cycle);
...
void RRTNoInterpolation::getMassCenter(base::State *state, double &x, double &y)
{
  x = 0.0;
  y = 0.0;
  auto statetype = static_cast<ob::CompoundStateSpace::StateType*>(state);
...
bool RRTNoInterpolation::checkTranslateValidity(base::State *source, base::State *destination)
{
  //return (getMaxEdge(destination) - fabs(getMaxEdge(source) - ) <= fmax(getEpsilon(source), getEpsilon(destination)));
  //return true;
  //return getCycleSize(destination) == si_->getStateSpace()->as<ob::CompoundStateSpace>()->getSubspaceCount();
  ...
  for (int i = 0; i < ncoords; ++i)
  {
    ...
    //distance = sqrt(pow((xS - xD), 2) + pow((yS - yD), 2)) - translationLength;
...
void RRTNoInterpolation::setGoalRegionRect(double x1, double y1, double x2, double y2)
{
  goalX1 = fmin(x1, x2);
  goalX2 = fmax(x1, x2);
  goalY1 = fmin(y1, y2);
...
bool RRTNoInterpolation::goalRegionIsSatisfied(ompl::base::State* state)
{
  bool sat = true;
  auto statetype = static_cast<ob::CompoundStateSpace::StateType*>(state);
  for (int i = 0; i < si_->getStateSpace()->as<ob::CompoundStateSpace>()->getSubspaceCount(); ++i)
...
void RRTNoInterpolation::setConfigRegistrator(std::function<void(ompl::base::State*)> reg)
{
  reg_ = reg;
}

void RRTNoInterpolation::addConfig(ompl::geometric::RRT::Motion* motion)
{
  Config conf = {};
  conf.motion = motion;
...
int RRTNoInterpolation::findConfigByRandomSeed(double w, int left, int right)
{
  if (left == right)
    return left;
...
int RRTNoInterpolation::findConfigByRandomSeed(double w)
{
  return findConfigByRandomSeed(w, 0, configurations.size() - 1);
}

RRTNoInterpolation::Config RRTNoInterpolation::pickConfig()
{
  RNG rng;
...
base::PlannerStatus RRTNoInterpolation::solve(const base::PlannerTerminationCondition &ptc)
{
  checkValidity();
  ...
  while (ptc == false)
  {
    ...
    if (si_->checkMotion(nmotion->state, dstate))
    {
      ...
      if (checkTranslateValidity(nmotion->state, dstate))
      {
        ...
        bool sat = goalRegionIsSatisfied(motion->state); // isSatisfied(motion->state, &dist);
        if (sat)
        {
          solution = motion;
          break;
